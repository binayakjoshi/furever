...
import PetOwnerProfileAction from "@/components/pet-owner/pet-owner-profile-actions";

type PetOwnerProfilePageProps = {
  params: Promise<{ slug: string }>;
};
const PetOwnerProfilePage = async ({ params }: PetOwnerProfilePageProps) => {
  const { slug } = await params;
  let user: PetOwner;
  try {
    const cookieHeader = (await cookies()).toString();
    const res = await fetch(
      `${process.env.NEXT_ROUTE_URL}/api/pet-owners/${slug}`,
      {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.profileCard}>
        ...
        <PetOwnerProfileAction userId={slug} />
      </div>
    </div>
  );
};
